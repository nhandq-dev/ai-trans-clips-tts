"""HMAC request signing for the pipeline worker.

Mirrors the video/TTS workers' scheme so one ``HMAC_KEYS_JSON`` rotation covers
all three services, but with ``X-Pipeline-*`` headers so a signature minted for
one service can never be replayed against another.
...
KEY_ID_HEADER = "X-Pipeline-Key-Id"
TIMESTAMP_HEADER = "X-Pipeline-Timestamp"
NONCE_HEADER = "X-Pipeline-Nonce"
CONTENT_SHA256_HEADER = "X-Pipeline-Content-SHA256"
