...
Thin wrapper around ``boto3`` so ``plan/009 §5.2`` stays storage-agnostic: only
``put_object``, ``get_object`` and ``generate_presigned_url`` are used. Changing
provider is env-only (see .env.example).

Used by both ``pipeline-worker`` and ``video-worker`` (the latter currently
streams files through Vercel and will migrate to presigned URLs in T0.4).
"""
...
def upload_file(local_path: str | Path, key: str, content_type: str | None = None) -> str:
    """Upload a local file to ``s3://{bucket}/{key}``. Returns the key."""
    client = _client()
    extra = {}
    if content_type:
...
def presign_get(key: str, ttl_seconds: int | None = None) -> str:
    """Presigned GET URL (worker signs, browser downloads directly)."""
    client = _client()
...
def presign_put(key: str, ttl_seconds: int | None = None, content_type: str | None = None) -> str:
    """Presigned PUT URL (browser uploads directly, bypassing Vercel)."""
    client = _client()
